...
class App {
  ...
  private scalingStrokeCheckbox: HTMLInputElement;
  ...
  private updateUrl = () => {
    ...
    urlSearchParams.set(
      'input-separate',
      String(this.separateCheckbox.checked)
    );
    urlSearchParams.set(
      'input-scaling-stroke',
      String(this.scalingStrokeCheckbox.checked)
    );
  ...
  init() {
    ...
    this.scalingStrokeCheckbox = this.$(
      '#input-scaling-stroke'
    ) as HTMLInputElement;
    ...
    this.copyToClipboardBtn = this.$(
      '#copy-to-clipboard-btn'
    ) as HTMLButtonElement;
    this.dummy = this.$('#dummy') as HTMLInputElement;

    // Init units select.
    Object.values(makerjs.unitType).forEach((unit) =>
      this.addOption(this.selectUnits, unit)
    );
  }
